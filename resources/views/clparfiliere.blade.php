...
@section('content')
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel" style="font-weight: bold;">Modifier Une Classe</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <form action="" id="modifierform">
              <div class="form-group">
                  @csrf
                  <label for="codeinput">Code Classe</label>
                  <input type="text" id="codeinput" class="form-control">
              </div>
              <div class="form-group">
                  <label for="filierechoice2">Filiere</label>
                  <select name="" id="filierechoice2" class="form-control">
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-success" id="modifierclasse">Modifier</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="modal fade" id="supprimer" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header" style="background: red; height:60px;">
          <h5 class="modal-title" id="exampleModalLabel" style="font-weight: bold; color: white;">Supprimer Une classe</h5>
        </div>
        <div class="modal-body suptext">
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Fermer</button>
          <button type="button" class="btn btn-danger" id="supprimerclasse">Supprimer</button>
        </div>
      </div>
    </div>
  </div>
...
@section('javascript')
    ...
    <script>
        $(document).ready(function(){
            $.ajax({
                url : "{{ URL('/admin/classeparfiliere/affichefiliere') }}",
                type : "POST",
                data : {
                    _token : "{{ csrf_token() }}"
                },
            ...
            $.ajax({
                url : "{{ URL('/admin/classeparfiliere/afficheclasse') }}",
                type : "POST",
                data : {
                    _token : "{{ csrf_token() }}"
                },
        ...
        $(document).on('change', '#filierechoice', function(){

            if($(this).find(':selected').val() == 'Open this select menu'){
                $.ajax({
                url : "{{ URL('/admin/classeparfiliere/afficheclasse') }}",
                type : "POST",
                data : {
                ...
                $.ajax({
                    url : "{{ URL('/admin/classeparfiliere/search') }}",
                    type : "POST",
                    data : {
                        _token : "{{ csrf_token() }}",
                        id : $(this).find(':selected').attr('name')
        ...
        $(document).on('click', '.supprimer', function(e){
            ...
            sessionStorage.setItem('idclasse', $(this).parent().closest('tr').find('th').eq(0).text());

            $(".suptext").html(
                "Voullez vous vraiment supprimer la classe <span style='font-weight: bold;'>" +  $(this).parent().closest('tr').find('td').eq(0).text() + "</span>"
            );
        ...
        $(document).on('click', '.modifier', function(e){
            e.preventDefault();
            $("#codeinput").val($(this).parent().closest('tr').find('td').eq(0).text());
            selectelement($("#filierechoice2"), "id", $(this).parent().closest('tr').find('td').eq(1).attr('name'));
            sessionStorage.setItem('idclasse', $(this).parent().closest('tr').find('th').eq(0).text());
        ...
        $(document).on('click', '#modifierclasse', function(e){
            ...
            $.ajax({
                url: "{{ URL('/admin/classeparfiliere/updateclasse') }}",
                type: "POST",
                data: {
                    _token: '{{ csrf_token() }}',
                    id : sessionStorage.getItem('idclasse'),
                    code: $("#codeinput").val(),
                    id_filiere: $("#filierechoice2").find(':selected').attr('name')
                },
                dataType: 'json',
                success: function(data){
                    $('.table').DataTable().destroy();
        ...
        $(document).on('click', '#supprimerclasse', function(e){
            ...
            $.ajax({
                url : "{{ URL('/admin/classeparfiliere/deleteclasse') }}",
                type: "POST",
                data: {
                    _token : '{{ csrf_token() }}',
                    id: sessionStorage.getItem('idclasse')
        ...
        function remplirselect(data){
                ...
                $("#filierechoice").append(ligne);
                $("#filierechoice2").append(ligne);

        }
        function remplir(data){
                var tableau = $("#classecontent");
                var ligne = "";

                for(let i=0 ; i<data.length ; i++){
                    ligne += "<tr>";
                    ligne += "<th>" + data[i].id + "</th>";
                    ligne += "<td>" + data[i].code + "</td>";
                    ligne += '<td name="' + data[i].id_filiere + '">' + data[i].codef + "</td>";
                    ligne += '<td><button class="btn btn-danger supprimer" type="button" data-toggle="modal" data-target="#supprimer">Supprimer</button></td>';
                    ligne += '<td><button class="btn btn-primary modifier" type="button" data-toggle="modal" data-target="#modifier">Modifier</button></td></tr>';
                }
        ...
        function selectelement(selector, type , value){
                if(type == "id"){
